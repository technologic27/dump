from get_embeddings import createBatches, createMatrices, iterate_minibatches, addCharInformation, padding
import pickle
...
class CNN_BLSTM():
	...
	def loadData(self, path_to_sentences):
		"""
		training: 60%
		dev: 20%
		test: 20%
		...
		with open(path_to_sentences, 'r') as iFile:
		...
		Training = int(0.6*no_of_sentences)
		Dev = int(0.2*no_of_sentences)
		Test = int(0.2*no_of_sentences)

		self.trainSentences = sentences[:Training]
		self.devSentences = sentences[Training:Training+Dev]
		self.testSentences = sentence[Traing+Dev:]
    ...
    def embed(self):
	    ...
	    fEmbeddings = open("../resources/glove.6B.50d.txt", encoding="utf-8")
	    ...
	    for line in fEmbeddings:
	        ...
	        if len(word2Idx) == 0:  # add padding+unknown
	            ...
	            self.wordEmbeddings.append(vector) #for unknown token, give a uniform distribution

	        if split[0].lower() in words: #only if word is in the embedding vector
    ...
    def buildModel(self):
        ...
        embed_char_out = TimeDistributed(Embedding(len(self.char2Idx), 30, embeddings_initializer=RandomUniform(minval=-0.5, maxval=0.5)), name="Character_embedding")(character_input)
        ...
        plot_model(self.model, to_file='model_output/model.png')
    ...
    def train(self):
        ...
        self.predLabels_test_history = []
        self.correctLabels_test_history = []

        self.predLabels_dev_history = []
        self.correctLabels_dev_history = []
        ...
        #save the f1, predLabels, correctLabels
        ...
        with open("model_output/" + self.modelName + "_predLabels_test.pkl", "w") as oFile:
            pickle.dump(self.predLabels_test_history, oFile)

        with open("model_output/" + self.modelName + "_correctLabels_test.pkl", "w") as oFile:
            pickle.dump(self.correctLabels_test_history, oFile)

        with open("model_output/" + self.modelName + "_predLabels_dev.pkl", "w") as oFile:
            pickle.dump(self.predLabels_dev_history, oFile)

        with open("model_output/" + self.modelName + "_correctLabels_dev.pkl", "w") as oFile:
            pickle.dump(self.correctLabels_dev_history, oFile)

        with open("model_output/" + self.modelName + "_f1_test.pkl", "w") as oFile:
            pickle.dump(self.f1_test_history, oFile)

        with open("model_output/" + self.modelName + "_f1_dev.pkl", "w") as oFile:
            pickle.dump(self.f1_dev_history, oFile)


        modelpath = "model_output/" + self.modelName + ".h5"
